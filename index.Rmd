...
study_hours_sat <- mutated_joined %>% group_by(study_hours_tier, sex_str) %>% summarize(avg_sat_sum=mean(sat_sum), med_sat_sum=median(sat_sum), sd_sat_sum=sd(sat_sum)) %>% knitr::kable(digits = 2, align = 'c', col.names=str_to_title(names(.)))
study_hours_sat
```
